...
ashita.events.register('command', 'command_cb', function (e)
    ...
    if (#args == 2 and args[2]:any('options')) then
        --process_incoming_message('討伐対象1：Velociraptor……7\r\n討伐対象2：Sand Cockatrice……3')
        process_incoming_message('討伐対象1：マンドラゴラ族……6\r\n討伐対象2：Sand Cockatrice……3')
        return;
    end

    if (#args == 2 and args[2]:any('accepted')) then
        process_incoming_message('訓練メニューを決定した')
        return;
    end

    if (#args == 2 and args[2]:any('confirmed')) then
        trainmon.target_zone_id = tonumber(AshitaCore:GetMemoryManager():GetParty():GetMemberZone(0))
        --process_incoming_message('討伐対象1：Velociraptor……0/7\r\n討伐対象2：Sand Cockatrice……0/3')
        process_incoming_message('討伐対象1：マンドラゴラ族……0/6\r\n討伐対象2：Sand Cockatrice……0/3')
        return;
    end

    if (#args == 2 and args[2]:any('cancelled')) then
        process_incoming_message('訓練メニューをキャンセルした')
        return;
    end

    if (#args == 2 and args[2]:any('killed')) then
        -- process_incoming_message('討伐対象のモンスターを倒しました(1/7)')
        -- process_incoming_message('Velociraptorを倒した')
        
        process_incoming_message('討伐対象のモンスターを倒しました(4/4)')
        process_incoming_message('Tenzenは、Beach Bunnyを倒した')
        return;
    end

    if (#args == 2 and args[2]:any('completed')) then
        process_incoming_message('訓練メニューを完遂した')
        return;
    end

    if (#args == 2 and args[2]:any('repeated')) then
        process_incoming_message('同じ訓練メニューを継続します')
        return;
    end
end)
